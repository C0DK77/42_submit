...
/*   Updated: 2025/09/29 09:42:51 by codk             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_env_vars_check(t_envp **env)
{
	int		i;
	t_var	*vars;
	...
	while (vars[i].var)
	{
		if (!ft_env_list_var_check(env, vars[i].var))
...
int	ft_env_list_var_check(t_envp **env, char *v)
{
	t_envp	*t;
	char	c[PATH_MAX];
	...
	if (!t)
		return (0);
	if (!ft_env_vars_create(env, t, v))
		return (free(t), 0);
	return (1);
}

int	ft_env_vars_create(t_envp **env, t_envp *t, char *v)
{
	char	cwd[PATH_MAX];
...
int	ft_var_increase_shlvl(t_envp *env)
{
	char	*t;

	t = ft_itoa(ft_atoi(env->value) + 1);
	if (!t)
		return (0);
	free(env->value);
	env->value = t;
